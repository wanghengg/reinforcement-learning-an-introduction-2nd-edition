...
class State:
    ...
    # computer the hash value for the state, it's unique
    def hash(self):
...
class Player:
    ...
    def __init__(self, step_size=0.1, epsilon=0.1):
        ...
        self.states = []  # 保存已经经历过的所有状态
        self.greedy = []  # 保存每个动作选择是否使用greedy-method
        self.symbol = 0
        ...
        for hash_val in all_states:
            ...
            if is_end:  # 判断游戏是否结束
                if state.winner == self.symbol:
                    ...
                    # if state.winner == 0, it's a tie
                    # 平局收益信号为0.5
                    self.estimations[hash_val] = 0.5
                else:
                    # 输掉比赛收益信号为0
                    self.estimations[hash_val] = 0
            else:  # 如果游戏没有结束，那么收益信号为0.5
                self.estimations[hash_val] = 0.5
    ...
    def backup(self):
        ...
        for i in reversed(range(len(states) - 1)):
            state = states[i]
            # 价值函数更新
            td_error = self.greedy[i] * (self.estimations[states[i + 1]] -
                                         self.estimations[state])
            self.estimations[state] += self.step_size * td_error

    # choose an action based on the state
    def act(self):
        state = self.states[-1]  # 当前状态
        next_states = []  # 保存此后所有可能的状态
        next_positions = []  # 保存此后所有可能落子的position
        for i in range(BOARD_ROWS):
            for j in range(BOARD_COLS):
                if state.data[i, j] == 0:
                    next_states.append(
...
class Judger:
    ...
    # @print_state: if True, print each board during game
