...
from exercise import constants as c
import numpy as np
import pandas as pd

from exercise.bandits import SampleAverageEstimator, \
    ActionValueBanditAgent, \
    EpsilonGreedyActor
from exercise.bandits import utils
...
def save_frame(frame, filename):
    return frame.to_pickle(os.path.join(c.Paths.output, 'ex_2_5', filename))
...
def new_agent():
    return ActionValueBanditAgent(
        estimators=[
            ...
            # ExponentialRecencyWeightedEstimator(step_size=ALPHA,
            #                                     initial_value=INITIAL_VALUE)
            for _ in range(N_BANDITS)
        ],
        actor=EpsilonGreedyActor(epsilon=EPSILON,
                                 n_actions=N_BANDITS,
...
if __name__ == '__main__':
    ...
    save_frame(
        all_choices,
        r'choices_{}_eps{}.pkl'.format(EstimatorType.__name__.lower(),
                                       EPSILON))

    save_frame(
        all_explore,
        r'explore_{}_eps{}.pkl'.format(EstimatorType.__name__.lower(),
                                       EPSILON))
